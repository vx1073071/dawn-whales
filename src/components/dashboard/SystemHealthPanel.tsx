  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-gray-400">{onlineCount}/{totalCount}{i18n.t("SystemHealthPanel.r92_4dc9")}</span>
      </div>);
  ...
  return (
    <div className="bg-[#1a1a25] border border-white/5 rounded-xl p-5 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-white font-semibold text-sm">{i18n.t("SystemHealthPanel.r92_02dc")}</h2>
          <p className="text-gray-500 text-xs mt-0.5">
            {onlineCount}/{totalCount}{i18n.t("SystemHealthPanel.r92_d0c8")}{formatUptime(health.uptimeSeconds)}
          </p>
        </div>
      ...
      <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3 flex items-center gap-2">
          ...
          <p className="text-red-300 text-xs">
            {offlineEngines.map((e) => e.name).join(', ')}{i18n.t("SystemHealthPanel.r92_558a")}
        </p>
        </div>
      }
      ...
      <div className="flex items-center justify-between text-xs text-gray-600 pt-2 border-t border-white/5">
        ...
        <button
          onClick={refresh}
          className="text-gray-500 hover:text-gray-300 transition-colors">{i18n.t("SystemHealthPanel.r92_fac8")}
