        ...
        {stats && (
          <div className="flex items-center gap-4 text-[10px] font-mono">
            <span className="text-gray-500">
              总收益 <span className={stats.totalReturn >= 0 ? 'text-emerald-400' : 'text-red-400'}>
                {stats.totalReturn >= 0 ? '+' : ''}{stats.totalReturn.toFixed(1)}%
              </span>
            </span>
            <span className="text-gray-500">
              最大回撤 <span className="text-red-400">{stats.maxDD.toFixed(1)}%</span>
            </span>
            <span className="text-gray-500">
              年化 <span className="text-[#D4A853]">{stats.annualized.toFixed(1)}%</span>
            </span>
          </div>
        )}
      </div>
