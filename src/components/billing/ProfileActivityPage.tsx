    ...
    <div className={`h-full flex flex-col bg-[#0D0D14] text-white ${className}`}>
      ...
      <div className="p-5 border-b border-white/5">
        ...
        <div className="grid grid-cols-5 gap-2">
          ...
          <StatCard value={profile.strategies} label="策略" />
          <StatCard value={profile.signals} label="信号" />
            ...
            {myStrategies.map(s => (
              <div key={s.id} className="bg-[#111119] border border-white/5 rounded-xl p-4">
                ...
                <div className="grid grid-cols-4 gap-2 text-center text-xs">
                  <div><div className="text-gray-500">收益</div><div className={`font-semibold ${s.return_ >= 0 ? 'text-green-400' : 'text-red-400'}`}>+{s.return_}%</div></div>
                  <div><div className="text-gray-500">夏普</div><div className="font-semibold text-gray-300">{s.sharpe}</div></div>
