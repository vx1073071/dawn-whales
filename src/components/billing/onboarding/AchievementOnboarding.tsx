  ...
  return (
    ...
    onClick={(e) => {if (e.target === e.currentTarget) onClose();}}>
      <div className="bg-[#1A1A24] border border-white/10 rounded-2xl p-8 text-center max-w-sm mx-4 shadow-2xl animate-bounce-in">
        ...
        <div className="text-lg font-bold text-white mb-2">{i18n.t("AchievementOnboarding.r92_7790")}</div>
        <div className="text-xl font-black mb-1" style={{ color: achievement.color }}>{achievement.title}</div>
        <p className="text-sm text-gray-400 mb-4">{achievement.description}</p>
        <button onClick={onClose}
        className="px-8 py-2.5 rounded-xl bg-[#C9A046] hover:bg-[#D4A853] text-black font-bold text-sm">{i18n.t("AchievementOnboarding.r92_c431")}
    ...
    <div className={`h-full flex flex-col bg-[#0D0D14] text-white ${className}`}>
      <div className="p-5 border-b border-white/5">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold">{i18n.t("AchievementOnboarding.r92_5525")}</h2>
            <p className="text-gray-500 text-xs mt-0.5">{unlocked}/{achievements.length}{i18n.t("AchievementOnboarding.r92_3d8f")}</p>
          </div>
          <div className="flex gap-1">
            <button onClick={() => setTab('achievements')}
            className={`px-3 py-1.5 rounded-md text-xs font-medium ${tab === 'achievements' ? 'bg-[#C9A046]/20 text-[#D4A853]' : 'text-gray-600'}`}>{i18n.t("AchievementOnboarding.r92_ec7d")}

            </button>
            <button onClick={() => setTab('onboarding')}
            className={`px-3 py-1.5 rounded-md text-xs font-medium ${tab === 'onboarding' ? 'bg-[#C9A046]/20 text-[#D4A853]' : 'text-gray-600'}`}>{i18n.t("AchievementOnboarding.r92_1733")}
      ...
      <div className="flex-1 overflow-y-auto p-5">
        ...
        <div className="space-y-4">
          ...
          <div key={s.step} className="bg-[#111119] border border-white/5 rounded-xl p-5">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold" style={{ color: s.done ? '#4ade80' : '#94a3b8' }}>{i18n.t("AchievementOnboarding.r92_56cd")}{s.step}{i18n.t('AchievementOnboarding.k0')}{s.title}</span>
                    </div>
            ...
            <div className="text-center text-[10px] text-gray-600">{i18n.t("AchievementOnboarding.r92_4758")}
