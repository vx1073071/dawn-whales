...
{ label: '200-1000 USDT', min: 200, max: 1000 }] as
const;
...
}: {strategy: StrategyCard;userBalance: number;onPurchase: (id: string, price: number) => Promise<{success: boolean;txHash?: string;}>;onClose: () => void;}) {
    ...
    onClick={(e) => {if (e.target === e.currentTarget) onClose();}}>
      <div className="bg-[#1A1A24] border border-white/10 rounded-xl w-full max-w-md mx-4 overflow-hidden shadow-2xl">
        ...
        <>
            ...
            <div className="p-2 bg-red-500/10 border border-red-500/20 rounded text-red-400 text-xs">{i18n.t("StrategyMarketplace.r92_970c")}
            ...
            className="flex-1 py-2.5 rounded-lg border border-white/10 text-gray-400 hover:text-white hover:border-white/20 text-sm transition-colors">{i18n.t("StrategyMarketplace.r92_3b64")}

            </button>
              <button onClick={handleBuy}
            disabled={userBalance < strategy.price}
            className="flex-1 py-2.5 rounded-lg bg-[#C9A046] hover:bg-[#D4A853] text-black font-semibold text-sm transition-colors disabled:opacity-40 disabled:cursor-not-allowed">{i18n.t("StrategyMarketplace.r92_c6ba")}
              {strategy.price} USDT
              </button>
            </div>
          </>
        ...
        <div className="p-10 flex flex-col items-center gap-4">
            ...
            <button onClick={onClose}
          className="mt-2 px-8 py-2.5 rounded-lg bg-[#C9A046] hover:bg-[#D4A853] text-black font-semibold text-sm transition-colors">{i18n.t("StrategyMarketplace.r92_4dc1")}
          ...
          <div className="bg-[#1A1A24] border border-white/10 rounded-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex flex-wrap gap-2 text-xs text-gray-500">
                <span>👥 {detailStrategy.subscribers.toLocaleString()}{i18n.t("StrategyMarketplace.r92_9410")}</span>
                <span>🔔 {detailStrategy.totalSignals}{i18n.t("StrategyMarketplace.r92_f19d")}</span>
                <span>📅 {detailStrategy.updatedAt}{i18n.t("StrategyMarketplace.r92_a18e")}</span>
