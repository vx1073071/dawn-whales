...
export const StrategyComparer: React.FC<StrategyComparerProps> = ({ className }) => {
  ...
  const radarData = useMemo(() => {
    const metrics: { key: string; label: string; maxVal: number; get: (s: StrategySnapshot) => number }[] = [
      ...
      { key: 'return', label: '收益', maxVal: 0.6, get: s => s.totalReturn },
      { key: 'drawdown', label: '回撤', maxVal: 0.25, get: s => Math.abs(s.maxDrawdown) },
      { key: 'winRate', label: '胜率', maxVal: 0.8, get: s => s.winRate },
    ];
  ...
  const metrics = useMemo(() => {
    const list: { label: string; valueA: number; valueB: number; format: (v: number) => string; invert?: boolean }[] = [
      ...
      { label: '总收益', valueA: strategyA.totalReturn, valueB: strategyB.totalReturn, format: v => `${(v * 100).toFixed(1)}%` },
      { label: '最大回撤', valueA: strategyA.maxDrawdown, valueB: strategyB.maxDrawdown, format: v => `${(v * 100).toFixed(1)}%`, invert: true },
      { label: '胜率', valueA: strategyA.winRate, valueB: strategyB.winRate, format: v => `${(v * 100).toFixed(1)}%` },
      ...
      { label: '盈亏比', valueA: strategyA.profitFactor, valueB: strategyB.profitFactor, format: v => v.toFixed(1) },
      ...
      <div className="grid grid-cols-2 gap-4 mb-5">
        ...
        <div className="bg-gray-800/40 rounded-lg p-3 border border-gray-700/30">
          ...
          <div className="grid grid-cols-3 gap-1 mt-2 text-center text-[10px]">
            ...
            <div><span className="text-gray-600">胜率</span><br/><span className="text-gray-400">{(strategyA.winRate * 100).toFixed(0)}%</span></div>
          </div>
        </div>
        {/* Strategy B */}
        <div className="bg-gray-800/40 rounded-lg p-3 border border-gray-700/30">
          ...
          <div className="grid grid-cols-3 gap-1 mt-2 text-center text-[10px]">
            ...
            <div><span className="text-gray-600">胜率</span><br/><span className="text-gray-400">{(strategyB.winRate * 100).toFixed(0)}%</span></div>
          </div>
        </div>
      </div>
