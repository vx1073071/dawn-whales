...
export default function FundHoldingsPage() {
  ...
  const fetchByFund = async () => {
    ...
    try {
      ...
      if (res?.success) setHoldings(res.items || []);else
      setError(res?.error || i18n.t('FundHoldingsPage.k1'));
    } catch (e: unknown) {
  ...
  const fetchByStock = async () => {
    ...
    try {
      ...
      if (res?.success) setOwnership(res.items || []);else
      setError(res?.error || i18n.t('FundHoldingsPage.k3'));
    } catch (e: unknown) {
  ...
  return (
    <div className="p-6 space-y-5 h-full overflow-auto">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-white mb-1">{i18n.t("FundHoldingsPage.r92_4587")}</h1>
      ...
      <div className="space-y-4">
          <div className="flex gap-3">
            ...
            onClick={fetchByFund}
            disabled={loading}
            className="bg-[#C9A046] hover:bg-[#b8933f] text-sidebar font-semibold px-5 py-2.5 rounded-lg text-sm transition-colors disabled:opacity-50">{i18n.t("FundHoldingsPage.r92_b872")}
          ...
          <div className="bg-[#1a1a25] border border-white/5 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tr key={h.stockCode} className="hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-3 text-right text-gray-300">{(h.shares / 10000).toFixed(1)}{i18n.t("FundHoldingsPage.r92_7efd")}</td>
                    <td className="px-4 py-3 text-right text-gray-300">{(h.marketValue / 10000).toFixed(0)}{i18n.t("FundHoldingsPage.r92_fb8b")}</td>
      ...
      <div className="space-y-4">
          <div className="flex gap-3">
            ...
            disabled={loading}
            className="bg-[#C9A046] hover:bg-[#b8933f] text-sidebar font-semibold px-5 py-2.5 rounded-lg text-sm transition-colors disabled:opacity-50">{i18n.t("FundHoldingsPage.r92_8716")}
          ...
          <div className="bg-[#1a1a25] border border-white/5 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tr key={o.stockCode + o.fundCount} className="hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-3 text-right text-gray-300">{(o.totalShares / 10000).toFixed(1)}{i18n.t("FundHoldingsPage.r92_6244")}</td>
      ...
      <div className="bg-[#1a1a25] border border-white/5 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tr key={f.fundCode} className="hover:bg-white/[0.02]">
                  ...
                  <td className="px-4 py-3 text-right text-gray-300">{(f.totalValue / 1e8).toFixed(1)}{i18n.t("FundHoldingsPage.r92_69a1")}</td>
                  <td className={`px-4 py-3 text-right font-medium ${f.changeValue >= 0 ? 'text-red-400' : 'text-emerald-400'}`}>
                    {(f.changeValue / 1e8).toFixed(1)}{i18n.t("FundHoldingsPage.r92_1ab1")}
              </td>
