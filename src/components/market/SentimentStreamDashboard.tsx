...
export default function SentimentStreamDashboard() {
  ...
  async function load() {
    ...
    } catch (e) {console.error('[Error:SentimentStreamDashboard]', e);}void EngineError; // [DATA] structured error tracking
    setLoading(false);}
  ...
  return (
    <div className="p-6 space-y-6 bg-deep min-h-full">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white mb-1">{i18n.t("SentimentStreamDashboard.r92_7024")}</h1>
          <p className="text-gray-400 text-sm">{i18n.t("SentimentStreamDashboard.r92_d26a")}</p>
        </div>
        <button
          onClick={load}
          className="text-xs bg-[#C9A046] hover:bg-[#D4A853] text-black font-medium px-4 py-2 rounded-lg transition-colors">{i18n.t("SentimentStreamDashboard.r92_9e0d")}
