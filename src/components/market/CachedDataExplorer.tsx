...
export default function CachedDataExplorer() {
  ...
  async function load() {
    ...
    try {
      ...
      // if (res?.success) { ... }
    } catch (e) {console.error('[Error:CachedDataExplorer]', e);}void EngineError; // [DATA] structured error tracking
    setLoading(false);}
  ...
  return (
    <div className="p-6 space-y-6 bg-deep min-h-full">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white mb-1">{i18n.t("CachedDataExplorer.r92_dc78")}</h1>
          <p className="text-gray-400 text-sm">{i18n.t("CachedDataExplorer.r92_67f0")}</p>
        </div>
        <button
          onClick={load}
          className="text-xs bg-[#C9A046] hover:bg-[#D4A853] text-black font-medium px-4 py-2 rounded-lg transition-colors">{i18n.t("CachedDataExplorer.r92_8597")}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div
          ...
          selectedNs === ns.name ? 'border-[#C9A046]' : 'border-white/5 hover:border-white/10'}`
            ...
            <div className="text-xs text-gray-500 mb-1">{ns.entries}/{ns.maxEntries}{i18n.t("CachedDataExplorer.r92_3675")}</div>
      ...
      <div className="bg-[#1a1a25] border border-white/5 rounded-xl p-5">
        <h2 className="text-sm font-semibold text-white mb-4">{i18n.t("CachedDataExplorer.r92_1f62")}</h2>
      ...
      <div className="flex gap-3">
        <button className="text-xs bg-[#22222f] hover:bg-[#2a2a3a] text-gray-300 px-4 py-2 rounded-lg border border-white/5 transition-colors">{i18n.t("CachedDataExplorer.r92_bc3d")}

        </button>
        <button className="text-xs bg-[#22222f] hover:bg-[#2a2a3a] text-gray-300 px-4 py-2 rounded-lg border border-white/5 transition-colors">{i18n.t("CachedDataExplorer.r92_2e1a")}

        </button>
        <button className="text-xs bg-red-500/10 hover:bg-red-500/20 text-red-400 px-4 py-2 rounded-lg border border-red-500/20 transition-colors">{i18n.t("CachedDataExplorer.r92_6ccf")}
