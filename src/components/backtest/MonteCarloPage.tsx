    ...
    <div className="min-h-screen bg-gray-900 text-white p-6 space-y-6">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-1">{t('投资期限:')}<span className="text-green-400 font-semibold">{t('{config.timeHorizon} 年')}</span></label>
            <input
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-md p-4 flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">{results.terminalValues.filter(v => v >{t('config.initialCapital).length} / {results.terminalValues.length} 次模拟盈利')}</p>
              </div>
            </div>
          </div>
