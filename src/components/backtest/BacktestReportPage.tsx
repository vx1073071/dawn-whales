            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-4 bg-[#12121a] rounded-xl border border-white/5">
                  ...
                  <div className="text-xs text-gray-600">{t('平均 +${result.avgWin.toFixed(2)}')}</div>
                </div>
                <div className="p-4 bg-[#12121a] rounded-xl border border-white/5">
                  <div className="text-xs text-gray-500 mb-1">{t('亏损次数')}</div>
                  <div className="text-lg font-bold text-red-400">{result.losingTrades}</div>
                  <div className="text-xs text-gray-600">{t('平均 -${Math.abs(result.avgLoss).toFixed(2)}')}</div>
                </div>
                <div className="p-4 bg-[#12121a] rounded-xl border border-white/5">
          ...
          {tab === 'equity' && (
            <div className="bg-[#12121a] rounded-xl border border-white/5 p-4">
              ...
              <div className="flex justify-center gap-6 mt-3 text-xs text-gray-500">
                <span>{t('初始: ${result.initialCapital.toLocaleString()}')}</span>
                <span>{t('最终: ${result.finalCapital.toFixed(2)}')}</span>
          ...
          {tab === 'trades' && (
            <div className="bg-[#12121a] rounded-xl border border-white/5 overflow-hidden">
                  ...
                  <tbody className="divide-y divide-white/[0.03]">
                    {sortedTrades.map((t) => (
                      <tr key={t.id} className="hover:bg-white/[0.02]">
                        ...
                        <td className="px-4 py-2.5 text-right text-gray-500">{t('{t.holdingDays}天')}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="px-4 py-2 border-t border-white/5 text-xs text-gray-600 flex justify-between">
                <span>{t('共 {result.trades.length} 笔交易')}</span>
                <span>{t('胜率 {(result.winRate * 100).toFixed(1)}% · 盈亏比 {result.profitLossRatio.toFixed(2)}')}</span>
              </div>
            </div>
          )}
