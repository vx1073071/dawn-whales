...
import { parameterSweep } from '../../lib/bridge-api';
...
export default function ParamScanPanel({ config }: { config: ParamSweepConfig }) {
  ...
  const paramKeys = Object.keys(config.paramRanges);
  ...
  async function runScan() {
    setLoading(true);
    try {
      const res = await parameterSweep({
  ...
  function buildHeatmap(data: ParamResult[]) {
    ...
    const avgSharpe = sharpeValues.reduce((s, v) => s + v, 0) / sharpeValues.length;

    for (const d of data) {
      const x = d.params[paramX] ?? 0;
      const y = d.params[paramY] ?? 0;
      // Robust: sharpe > avg and within top 20%
  ...
  const maxSharpe = Math.max(...results.map((r) => r.sharpeRatio), 0.1);
  const minSharpe = Math.min(...results.map((r) => r.sharpeRatio), 0);
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={runScan}
            disabled={loading}
            className="text-xs bg-[#C9A046]/20 text-[#D4A853] px-3 py-1 rounded-lg hover:bg-[#C9A046]/30 disabled:opacity-40"
          >
            {loading ? '扫描中...' : '重新扫描'}
          </button>
        </div>
