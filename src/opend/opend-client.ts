...
export class OpenDClient {
  ...
  private callbacks: Map<string, ((data: Record<string, unknown>) => void)[]> = new Map();
  ...
  on(type: string, callback: (data: Record<string, unknown>) => void): void {
    const handlers = this.callbacks.get(type) || [];
    handlers.push(callback);
    this.callbacks.set(type, handlers);
  }

  off(type: string, callback: (data: Record<string, unknown>) => void): void {
    const handlers = this.callbacks.get(type) || [];
    this.callbacks.set(
      type,
