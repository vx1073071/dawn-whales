// run-qtest.js - Run QTest demo tests
import { describe, it, expect, qmock, runFiles, printReport, setupGlobals } from './test-framework/qtest.js';
import { generateHtmlReport } from './test-framework/html-report.js';

// Make globals available
setupGlobals(globalThis);

// ===== Define tests directly =====
...
// ===== Run =====
async function main() {
  const result = await runFiles([]);  // tests already defined above
  printReport(result);
  // Also generate HTML dashboard
  const htmlPath = await generateHtmlReport(result, 'qtest-dashboard.html');
