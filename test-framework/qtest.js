...
export function describe(name, fn) {
  ...
  } else if (state.rootSuite && !state.rootSuite.name) {
    // Append to anonymous root
    state.rootSuite.children.push(suite);
    suite.parent = state.rootSuite;
  } else if (state.rootSuite) {
    // Create new anonymous root and add both old and new as children
    const newRoot = createSuite('(root)', state.currentFile);
    newRoot.children.push(state.rootSuite);
    state.rootSuite.parent = newRoot;
    newRoot.children.push(suite);
    suite.parent = newRoot;
    state.rootSuite = newRoot;
  } else {
    state.rootSuite = suite;
  }
...
export function createExpect(received, config = { not: false, soft: false }) {
  ...
  const toHaveBeenCalledTimes = makeMatcher((received, expected) => {
    ...
    return { pass: mock.mock.calls.length === expected, message: () => '.toHaveBeenCalledTimes(' + expected + ')' };
  });
  ...
  const toHaveBeenCalledWith = makeMatcher((received, expected) => {
    ...
    // expected can be an array of args, e.g. [1, 2] -> compare with call.args
    const pass = mock.mock.calls.some(call => {
      if (!Array.isArray(expected)) return getDeepEquality(call.args, [expected]);
      return getDeepEquality(call.args, expected);
    });
  ...
  const matchers = { toBe, toEqual, toBeTruthy, toBeFalsy, toBeNull, toBeUndefined, toBeDefined, toBeNaN, toBeGreaterThan, toBeLessThan, toContain, toHaveLength, toThrow, toHaveBeenCalled, toHaveBeenCalledTimes, toHaveBeenCalledWith, toHaveReturnedWith };
...
export async function runFiles(files, config = {}) {
  ...
  for (const file of files) {
    ...
    // Use file:// URL for import() to handle Windows paths
    const absPath = resolve(file);
    const fileUrl = pathToFileURL(absPath).href;

    try { await import(fileUrl); } catch (e) {
  ...
  // Also process any suites defined directly (not via file import)
  if (state.rootSuite && allSuites.length === 0) {
    const result = await runSuite(state.rootSuite);
    allSuites.push(result);
    for (const t of result.tests) { if (t.status === 'passed') passed++; else if (t.status === 'failed') failed++; else if (t.status === 'skipped') { if (t.todo) todo++; else skipped++; } }
  }
