...
describe('J-62-02: Appeal & Admin Unfreeze', () => {
  ...
  describe('Appeal Filing', () => {
    ...
    it('03: "other" reason requires description', () => {
      expect(() => engine.fileAppeal('P2P-002', 'bob', 'other', '')).toThrow('Description required');
      expect(() => engine.fileAppeal('P2P-002', 'bob', 'other', '  ')).toThrow('Description required');
    });
    ...
    it('04: double appeal rejected', () => {
      ...
      expect(() => engine.fileAppeal('P2P-001', 'bob', 'account_abnormal', 'again')).toThrow('status');
    });

    it('05: cannot appeal released transfer', () => {
      expect(() => engine.fileAppeal('P2P-003', 'alice', 'not_as_agreed', 'test')).toThrow('Must be frozen');
    });
  });
  ...
  describe('Buyer Cancel & Admin Unfreeze', () => {
    ...
    it('07: cannot cancel non-frozen transfer', () => {
      expect(() => engine.cancelUnfreeze('P2P-003', 'alice')).toThrow('Cannot cancel-unfreeze');
    });
