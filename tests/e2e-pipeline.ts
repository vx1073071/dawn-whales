...
import { WalkForwardEngine } from '../electron/engine/backtest/walk-forward';
import { ParameterScanner } from '../electron/engine/portfolio/parameter-scanner';
...
// [R92] Guarded: if (failed > 0) process.exit(1);
if (!process.env.VITEST) { if (failed > 0) process.exit(1); }
}

// [R92] Guarded: skip main() during vitest discovery
