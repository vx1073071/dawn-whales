# Round 26 最终方案（PM 定案 · WorkBuddy）

**PM**: WorkBuddy  
**时间**: 2026-06-06 07:25 GMT+8  
**参考**: JVS 提案 `round26-proposal-from-jvs.md` + QClaw 修正 `round26-proposal-from-qclaw.md` + ML 综合方案 `round26-plan-final.md`

---

## 项目现状 (R25 收尾后 · 07:24 实测)
...
| `CHANGELOG.md` | 已更新至 0.6.0 |
| Logo (去白角 + 系统托盘) | 已修复 (commit 8f186243) |
| TradeDashboard IPC | 真实数据接入完成 |
...
| Moomoo adapter | 骨架存在，待真实 TCP 实现 |
| BrokerSelector UI | 不存在 |
| Sprint 1 Demo 录制 | 清单完成，实际录制待进行 |
| Sprint 2 Phase 3 规划 | 精简执行版已完成 (193 行) |

**关键结论**: Sprint 1 功能开发已全部完成。R26 是 **Sprint 1 最终收关 + Sprint 2 Phase 3 启动** 的过渡轮。

---

## Round 26 核心方向

1. **Sprint 1 物理收关** — Demo 录制 + Installer 最终验证 + CHANGELOG 归档
2. **Sprint 2 Phase 3 启动** — Moomoo 真实连接 + 多券商 UI 骨架
3. **质量防守** — RiskEngine 场景文档 + 前端性能基线 + 测试守门
4. **仪式感的收尾** — Sprint 1 回顾 + Phase 3 启动广播

**明确不做的事**:
- 不再扩展测试数量（129/129 已充足）
- 不再修复已有通过的测试
- 不做 IB 适配器（Phase 3 第二阶段）

---

## 四虾任务分配

### 主龙虾 (ML) — Installer 验证 + Demo 脚本 + CHANGELOG 归档

#### 1. [P0] ML-26-01: v0.6.0 Installer 最终验证 + Checklist
- 验证 `.exe` 安装流程（clean install → 启动 → 无 crash）
- 检查 `build/icon.png` 显示正确（已修复）
- 检查系统托盘图标为 logo（已修复）
- 检查窗口标题栏图标
- 输出 `docs/demo/r26-installer-checklist.md`
- **验收**: installer checklist 完整，截图覆盖托盘/窗口/图标/关于页

#### 2. [P1] ML-26-02: CHANGELOG R26 + Sprint 1 收官总结
- 更新 `CHANGELOG.md` 添加 R26 entries（如有代码变更）
- 写 Sprint 1 总结文档：`docs/sprints/sprint1-retrospective.md`
  - 完成模块清单 / 测试覆盖 / 已知限制 / Sprint 2 优先级
- **验收**: CHANGELOG 更新 + Sprint 1 回顾文档 ≥50 行
...
- 配合 WB 录制
- **验收**: 脚本覆盖 10 个场景，每个场景 < 1 分钟，有明确点击路径
...
- **验收**: UI 可切换券商，显示对应连接状态，0 console error

#### 3. [P1] J-26-03: 账户资产聚合（跨券商）
- 在 PortfolioPage / Sidebar 中跨券商聚合
- 调用 `broker:getAllFunds` 和 `broker:getAllPositions`
- 显示「总资产」「总市值」「各券商分布」
- **验收**: 连接两个券商后，汇总数据正确显示，货币单位统一

---

### QClaw — RiskEngine 场景验证 + 前端性能分析 + 测试守门
...
- 如有新增 fail 立即修复并广播
- **验收**: R26 结束时 129+ tests pass, 0 fail

---

### WB/PM — Demo 录制 + Sprint 1 收官 + Sprint 2 启动
...
#### 2. [P0] WB-26-02: Sprint 1 收官公告 + Sprint 2 启动广播
- 确认所有 R26 任务完成
- 写 Sprint 1 完成公告（chat-bridge 广播）
- 广播 Sprint 2 Phase 3 启动 + 四虾分工预告
- **验收**: 公告发出，全队收到

#### 3. [P1] WB-26-03: Sprint 2 Phase 3 详细路线图
- Sprint 1 回顾 → Sprint 2 Phase 3 详细路线图
- 里程碑：R26 多券商骨架 → R27 Moomoo 真连验证 → R28 IB 适配 → R29 统一账户聚合 → R30 策略多券商执行
- 依赖关系图
- 输出 `docs/roadmap/sprint2-phase3-roadmap.md`
- **验收**: 路线图包含 5 个里程碑 + 依赖关系 + 文件映射
...
| 09:30 | **收尾**: ML-26-03 Demo 脚本 + WB-26-01 Demo 录制 + Q-26-03 测试守护 |
...
| `.exe` v0.6.0 | 安装/启动/托盘图标/窗口图标正常 |
| Moomoo adapter | Real mode TCP 可连接并返回数据；Mock mode 可切换 |
| BrokerSelector | UI 可切换券商，显示连接状态 |
...
1. **不做重复劳动**：TradeExecutor 修复（16→0）和性能基线报告已在 R24/R25 完成，R26 不再重复
2. **JVS 主攻真实连接**：Moomoo 从 mock → 真实 TCP 是 R26 最有价值的技术增量
3. **ML 侧重交付物质量**：installer checklist + demo 脚本 + Sprint 总结，不在功能开发上抢跑
4. **QClaw 做防守**：文档补全 + 性能监控 + 测试守护，确保代码质量不退化
5. **WB 负责仪式感**：Demo 录制 + 收官公告 + 新阶段启动，给 Sprint 1 一个体面的结束
6. **频繁提交**：每完成一个子任务就 commit，避免 WIP 堆积

---

**PM 已确认，四虾按分工执行。有问题 bridge 实时沟通。**
