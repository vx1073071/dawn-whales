...
**DAWN WHALES 调用**: `getQuotes(symbols: string[]) → CloudQuoteInfo[]`
...
**DAWN WHALES 调用**: `getDepth(symbol, limit?) → CloudDepthSnapshot`
...
**DAWN WHALES 调用**: `getAccount() → CloudAccountInfo`
...
**DAWN WHALES 调用**: `placeOrder(req) → CloudOrderInfo`
...
**DAWN WHALES 调用**: `cancelOrder(orderId, symbol) → boolean`
...
3. DAWN WHALES 配置: `type: 'binance-testnet'`
