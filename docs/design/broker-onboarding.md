# Dawn Whales 券商接入手册 — 从 API Key 到实时行情
...
Dawn Whales 的 futu-adapter.ts 负责 Protobuf ↔ JSON 转换。
...
Dawn Whales 的 ib-tws-adapter.ts 将 IB 私有协议转为 JSON WS。
