# Dawn Whales Fee Structure v17.6
...
Dawn Whales charges a **per-trade fee** for orders placed through the platform. Fees are deducted in USDT from the user's wallet **before** the order is sent to the broker. If the order fails, the fee is **refunded** to the wallet.
