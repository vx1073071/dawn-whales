...
quant-moo handles real USDT with a two-wallet architecture and server-side validation. This document covers all chain-level security threats and their mitigations.
...
- Address format validation before broadcast:
  ...
  ```
- **Cannot prevent**: Blockchain transactions are irreversible. quant-moo is non-custodial for withdrawals — user is responsible for address correctness.
