...
**操作步骤**:
1. 双击 `quant-moo Setup 0.6.0.exe` 启动应用（或 `npm start`）
2. 等待应用加载完成（约 3–5 秒）
...
**旁白建议**:
> 「这是 quant-moo 的 Dashboard，启动后自动加载。总资产 150 万港元，今日盈利 1.25 万。净值曲线展示了策略运行以来的累计收益走势。」
...
**操作步骤**:
1. 在文件资源管理器中定位 `quant-moo Setup 0.6.0.exe`
...
9. 勾选「Launch quant-moo」→ 点击「Finish」
10. 应用自动启动
...
**旁白建议**:
> 「quant-moo 提供 Windows 安装包。双击安装向导，选择路径后一键安装。安装完成后自动启动，桌面生成快捷方式。」
