...
// marketplace:* 相关的 IPC handlers
// 从 main.ts 拆分出来，约10个 handlers

import { ipcMain } from 'electron';
import { shared } from './_import-shared';
import { validate, MarketplaceRateSchema, MarketplaceCommentSchema, 
         MarketplaceSavePerformanceSchema, MarketplaceListSchema } from '../ipc-schemas';
...
export function registerMarketplaceHandlers() {
  ...
  ipcMain.handle('marketplace:list', async (_e, sortBy?: string, limit?: number) => {
    const vErr = validate(MarketplaceListSchema, { sortBy, limit });
    if (vErr) return vErr;
