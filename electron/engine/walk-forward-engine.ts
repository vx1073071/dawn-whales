...
export class WalkForwardEngine extends EventEmitter {
  ...
  private validateConfig(): void {
    const c = this.config;
    if (c.windows < 2) throw new Error('WalkForward: windows must be >= 2');
    if (c.inSampleRatio < 0.5 || c.inSampleRatio > 0.9) {
      throw new Error('WalkForward: inSampleRatio must be between 0.5 and 0.9');
    }
    if (c.minTrades < 1) throw new Error('WalkForward: minTrades must be >= 1');
    if (this.paramRanges.length === 0) {
      throw new Error('WalkForward: at least one parameter range is required');
    }
    for (const p of this.paramRanges) {
      if (p.step <= 0) throw new Error(`WalkForward: param "${p.name}" step must be > 0`);
      if (p.min > p.max) throw new Error(`WalkForward: param "${p.name}" min > max`);
    }
  }
  ...
  async run(data: KLine[]): Promise<WalkForwardResult> {
    if (this.running) throw new Error('WalkForward: engine is already running');
    if (data.length < 10) throw new Error('WalkForward: insufficient data (need >= 10 bars)');
  ...
  updateConfig(config: Partial<WalkForwardConfig>): void {
    if (this.running) throw new Error('WalkForward: cannot update config while running');
  ...
  private calculateWindowBoundaries(dataLength: number): Array<{
    ...
    if (segmentSize < 4) {
      throw new Error('WalkForward: not enough data for the number of windows requested');
    }
