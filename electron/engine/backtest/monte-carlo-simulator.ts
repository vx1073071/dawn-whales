...
export class MonteCarloSimulator {
  ...
  compareScenarios(scenarios: ScenarioResult[]): ScenarioComparison {
    if (scenarios.length === 0) {
      throw new EngineError(ErrorCode.INTERNAL_ERROR, '[MonteCarloSimulator] No scenarios to compare');
    }
  ...
  sensitivityAnalysis(
    ...
    if (!validParams.includes(paramName)) {
      throw new EngineError(ErrorCode.INTERNAL_ERROR, `[MonteCarloSimulator] Invalid parameter "${paramName}". ` +
  ...
  private validateConfig(config: SimConfig): void {
    if (config.initialCapital <= 0) {
      throw new EngineError(ErrorCode.INTERNAL_ERROR, `[MonteCarloSimulator] initialCapital must be > 0, got ${config.initialCapital}`,);
    }
    if (config.volatility < 0) {
      throw new EngineError(ErrorCode.INTERNAL_ERROR, `[MonteCarloSimulator] volatility must be >= 0, got ${config.volatility}`,);
    }
    if (config.horizon <= 0) {
      throw new EngineError(ErrorCode.INTERNAL_ERROR, `[MonteCarloSimulator] horizon must be > 0, got ${config.horizon}`,);
    }
    if (config.simulations < 1) {
      throw new EngineError(ErrorCode.INTERNAL_ERROR, `[MonteCarloSimulator] simulations must be >= 1, got ${config.simulations}`,);
    }
    ...
    if (!validDist.includes(config.distribution)) {
      throw new EngineError(ErrorCode.INTERNAL_ERROR, `[MonteCarloSimulator] Invalid distribution "${config.distribution}". ` +
