...
class RawSocket {
  ...
  constructor(url: string, protocols?: string[]) {
    ...
    this.ws.onclose = (ev: any) => this.onclose?.(ev);
    this.ws.onmessage = (ev: any) => this.onmessage?.(ev);
    this.ws.onerror = (ev: any) => this.onerror?.(ev);
  }
...
export class WsMarketDataEngine {
  ...
  private doConnect(): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      ...
      this.socket.onmessage = (ev: { data: any }) => {
  ...
  private sendMessage(payload: Record<string, any>): void {
  ...
  private processTickMessage(parsed: any): void {
  ...
  private parseTick(data: any): MarketTick | null {
  ...
  private processDepthMessage(parsed: any): void {
  ...
  private processKlineMessage(parsed: any): void {
