...
export class CreatorOnboardingServer {
  ...
  startOnboarding(userId: string, agent: AgentType): OnboardingProfile {
    if (this.profiles.has(userId)) throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, 'Onboarding already in progress');
  ...
  setAgentParams(userId: string, params: Partial<AgentParams>, temperament?: AgentTemperament): OnboardingProfile {
    ...
    if (profile.currentStep !== 1 && profile.currentStep !== 2) throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, `Invalid step ${profile.currentStep}`);
  ...
  submitBacktest(userId: string, result: BacktestResult): OnboardingProfile {
    const profile = this.profiles.get(userId);
    if (!profile) throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, 'Onboarding not started');
    if (profile.currentStep !== 2) throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, `Invalid step ${profile.currentStep}`);

    // Validate backtest
    if (result.totalTrades < 10) throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, 'Backtest requires at least 10 trades');
    if (result.winRate < 0) throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, 'Invalid win rate');
  ...
  configureSignal(userId: string, config: { name: string; description: string; tier: SignalTier; interval: string }): OnboardingProfile {
    const profile = this.profiles.get(userId);
    if (!profile) throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, 'Onboarding not started');
    if (profile.currentStep !== 3) throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, `Invalid step ${profile.currentStep}`);
  ...
  publishSignal(userId: string): { profile: OnboardingProfile; signal: SignalConfig } {
    const profile = this.profiles.get(userId);
    if (!profile) throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, 'Onboarding not started');
    if (profile.currentStep !== 4) throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, `Invalid step ${profile.currentStep}`);
    if (!profile.signal) throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, 'Signal not configured');
