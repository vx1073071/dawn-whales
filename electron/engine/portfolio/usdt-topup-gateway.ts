...
export class USDTTopupGateway extends EventEmitter {
  ...
  initiateTopup(
    ...
    if (amountUSDT <= 0) throw new EngineError(ErrorCode.INTERNAL_ERROR, 'Amount must be positive');
  ...
  confirmTopup(recordId: string): TopupRecord {
    const record = this.topupRecords.find(r => r.id === recordId);
    if (!record) throw new EngineError(ErrorCode.INTERNAL_ERROR, `Topup not found: ${recordId}`);
  ...
  internalTransfer(fromCreator: string, toCreator: string, amountUSDT: number, description?: string): InternalTransfer {
    if (amountUSDT <= 0) throw new EngineError(ErrorCode.INTERNAL_ERROR, 'Amount must be positive');
    if (fromCreator === toCreator) throw new EngineError(ErrorCode.INTERNAL_ERROR, 'Cannot transfer to self');
