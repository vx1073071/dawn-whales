...
function extractReturns(data: unknown[]): number[] {
...
function extractFactorReturns(factorData: unknown[]): number[] {
...
export class PerformanceAttributionEngine extends EventEmitter {
   ...
   */
  attribute(performance: unknown[], benchmark: unknown[]): AttributionResult {
   ...
   */
  fitFactorModel(performance: unknown[], factors: unknown[][]): FactorModel {
   ...
   */
  computeRollingSharpe(performance: unknown[], window: number = 21): number[] {
    const sr = extractReturns(performance);
    const result: number[] = [];
   ...
   */
  computeInformationRatio(performance: unknown[], benchmark: unknown[]): number {
    const sr = extractReturns(performance);
    const br = extractReturns(benchmark);
    const len = Math.min(sr.length, br.length);
    if (len < 2) return 0;
   ...
   */
  computeCalmarRatio(performance: unknown[]): number {
    const sr = extractReturns(performance);
   ...
   */
  computeTailRatio(performance: unknown[]): number {
   ...
   */
  computeWinRate(performance: unknown[]): number {
    const sr = extractReturns(performance);
   ...
   */
  computeProfitFactor(performance: unknown[]): number {
    const sr = extractReturns(performance);
