...
export class DataVersionController {
  ...
  createSnapshot(
    ...
    if (!branch) {
      throw new Error(`Active branch "${this.currentBranch}" not found`);
    }
  ...
  diff(snapshotId1: string, snapshotId2: string): DataDiff {
    ...
    if (!stored1) {
      throw new Error(`Snapshot "${snapshotId1}" not found`);
    }
    if (!stored2) {
      throw new Error(`Snapshot "${snapshotId2}" not found`);
    }
  ...
  createBranch(name: string, fromSnapshotId?: string): BranchInfo {
    ...
    if (!this.snapshots.has(startId)) {
      throw new Error(`Snapshot "${startId}" not found`);
    }
  ...
  mergeBranch(sourceBranch: string, targetBranch: string): DataDiff {
    ...
    if (!sourceHead || !targetHead) {
      throw new Error('Head snapshot missing for source or target branch');
    }
   ...
   */
  getContentHash(data: any[]): string {
   ...
   */
  private buildRowKeyMap(data: any[]): Map<string, number> {
