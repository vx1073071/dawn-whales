...
export class RiskEngineV3 extends EventEmitter {
  ...
  async aggregateAccounts(options: {
    ...
    for (const brokerId of options.brokerIds) {
      const adapter = this.brokerAdapters.find((a: unknown) => a.type === brokerId);
      if (!adapter) {
      ...
      try {
        ...
        for (const acc of accInfos) {
          ...
          const aggPositions: AggregatedPosition[] = (positions || []).map((p: unknown) => ({
            code: p.code,
            name: p.name || p.code,
            qty: p.qty || 0,
  ...
  async getPortfolioExposure(): Promise<PortfolioExposure> {
    ...
    const allBrokerIds = [...new Set(this.brokerAdapters.map((a: unknown) => a.type))];
  ...
  async checkCircuitBreaker(market: string): Promise<CircuitBreakerResult> {
    ...
    for (const adapter of this.brokerAdapters) {
      try {
        ...
        const found = (quotes || []).find((q: unknown) => q.code === indexCode);
        if (found) {
          quote = found;
          break;
        }
