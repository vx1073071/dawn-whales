...
export class DownloadRegistrationServer {
  ...
  register(req: RegisterRequest): UserAccount {
    // Validate
    if (!this.isValidEmail(req.email)) throw new Error('Invalid email format');
    if (this.emails.has(req.email.toLowerCase())) throw new Error('Email already registered');
    if (req.password.length < 8) throw new Error('Password too short (min 8 characters)');
