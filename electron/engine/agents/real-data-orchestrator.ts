...
export class RealDataOrchestrator {
  ...
  private normalizeQuote(data: unknown, source: string): MarketDataResult {
    // Proprietary format
    return {
      symbol: data.symbol,
      market: data.market,
  ...
  private normalizeYahooQuote(data: unknown, symbol: string, market: string): MarketDataResult {
  ...
  private normalizeAVQuote(data: unknown, symbol: string, market: string): MarketDataResult {
  ...
  private normalizeNewsApi(data: unknown): NewsItem[] {
    const articles = data?.articles ?? [];
    return articles.map((a: unknown, i: number) => ({
      id: `newsapi-${i}`,
      title: a.title ?? "",
  ...
  private normalizeProprietaryNews(data: unknown): NewsItem[] {
