...
export class GeneticAlgorithm {
  ...
  step(): void {
    if (!this.config) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] No configuration set. Call optimize() first.');
    }
    if (!this.initialized) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] Not initialized. Call optimize() first.');
    }
  ...
  getBestIndividual(): Individual {
    if (!this.bestIndividual) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] No best individual. Run at least one generation.');
    }
  ...
  private initializePopulation(): void {
    if (!this.config) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] No configuration set.');
    }
  ...
  private getBestInPopulation(): Individual {
    if (this.population.length === 0) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] Population is empty.');
    }
  ...
  private buildResult(durationMs: number): GAResult {
    if (!this.bestIndividual) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] No best individual found after optimization.');
    }
  ...
  private validateConfig(config: GAConfig): void {
    if (config.populationSize < 2) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] Population size must be at least 2.');
    }
    if (config.generations < 1) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] Generations must be at least 1.');
    }
    if (config.crossoverRate < 0 || config.crossoverRate > 1) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] Crossover rate must be between 0 and 1.');
    }
    if (config.mutationRate < 0 || config.mutationRate > 1) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] Mutation rate must be between 0 and 1.');
    }
    if (config.elitismCount < 0) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] Elitism count must be non-negative.');
    }
    if (config.elitismCount >= config.populationSize) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] Elitism count must be less than population size.');
    }
    if (config.tournamentSize < 1) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] Tournament size must be at least 1.');
    }
    if (config.tournamentSize > config.populationSize) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] Tournament size must not exceed population size.');
    }
    if (!config.genes || config.genes.length === 0) {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] At least one gene definition is required.');
    }
    if (typeof config.fitnessFunction !== 'function') {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] fitnessFunction must be a function.');
    }
  ...
  private validateGene(gene: Gene): void {
    if (!gene.name || gene.name.trim() === '') {
      throw new EngineError(ErrorCode.ML_ERROR, '[GA] Gene name must be a non-empty string.');
    }
    if (gene.min >= gene.max) {
      throw new EngineError(ErrorCode.ML_ERROR, `[GA] Gene "${gene.name}": min must be less than max.`);
    }
    if (gene.step <= 0) {
      throw new EngineError(ErrorCode.ML_ERROR, `[GA] Gene "${gene.name}": step must be positive.`);
    }
    if (gene.type !== 'int' && gene.type !== 'float') {
      throw new EngineError(ErrorCode.ML_ERROR, `[GA] Gene "${gene.name}": type must be 'int' or 'float'.`);
    }
    if (gene.step > gene.max - gene.min) {
      throw new EngineError(ErrorCode.ML_ERROR, `[GA] Gene "${gene.name}": step must not exceed the range (max - min).`);
    }
  }
