 ...
 *
 * Maximum absolute error ‚â?1.5 √ó 10‚Åª‚Å∑.
 */
...
export class OptionsPricingEngine {
   ...
   * and logs a warning ‚Ä?use `binomialTree` for proper American pricing.
   */
  public blackScholes(params: OptionParams): PricingResult {
    ...
    if (params.style === 'american') {
      log.warn(
        '[OptionsPricingEngine] blackScholes called with american style ‚Ä?' +
    ...
    const durationMs = nowMs() - start;
    log.debug(
      `[OptionsPricingEngine] BS ${params.type} S=${S} K=${K} T=${T} œÉ=${sigma} ‚Ü?price=${price.toFixed(4)} (${durationMs.toFixed(2)} ms)`,
    );
  ...
  public binomialTree(params: OptionParams, config?: BinomialConfig): PricingResult {
    ...
    if (N < 1 || !Number.isInteger(N)) {
      throw new Error(`BinomialConfig.steps must be a positive integer, got ${N}`);
    }
    ...
    const durationMs = nowMs() - start;
    log.debug(
      `[OptionsPricingEngine] CRR(${N}) ${params.style}/${params.type} S=${S} K=${K} T=${T} œÉ=${sigma} ‚Ü?price=${price.toFixed(4)} (${durationMs.toFixed(2)} ms)`,
    );
  ...
  public impliedVolatility(
    ...
    if (marketPrice <= 0) {
      throw new Error(`Market price must be positive, got ${marketPrice}`);
    }
    ...
    if (T <= 0) {
      throw new Error('Cannot compute implied volatility for expired options (T ‚â?0)');
    }
    ...
    if (marketPrice < intrinsic - 1e-6) {
      log.warn(
        `[OptionsPricingEngine] Market price ${marketPrice} is below intrinsic ${intrinsic.toFixed(6)} ‚Ä?IV may be unreliable`,
      );
    }
    ...
    for (let iter = 0; iter < IV_MAX_ITERATIONS; iter++) {
      ...
      if (Math.abs(diff) < IV_TOLERANCE) {
        log.debug(
          `[OptionsPricingEngine] IV converged in ${iter + 1} iterations ‚Ü?œÉ=${sigma.toFixed(6)}`,
        );
        return sigma;
      }
      ...
      if (Math.abs(vega) < 1e-12) {
        log.warn('[OptionsPricingEngine] Vega near zero ‚Ä?switching to bisection');
    ...
    // Newton-Raphson did not converge ‚Ä?fall back to bisection
    log.warn(
      '[OptionsPricingEngine] Newton-Raphson did not converge ‚Ä?falling back to bisection',
    );
    return this.impliedVolatilityBisection(marketPrice, params as Omit<OptionParams, 'volatility'>);
  }
   ...
   * `profit`  = payoff ‚à?premium paid (uses BS price as premium)
   */
  ...
  public putCallParity(callPrice: number, params: OptionParams): number {
    ...
    log.debug(
      `[OptionsPricingEngine] Put-Call parity: C=${callPrice.toFixed(4)} ‚Ü?P=${putPrice.toFixed(4)} (PV_fwd=${pvForward.toFixed(4)})`,
    );
  ...
  private validateParams(params: OptionParams): void {
    if (params.underlying <= 0) {
      throw new Error(`Underlying price must be positive, got ${params.underlying}`);
    }
    if (params.strike <= 0) {
      throw new Error(`Strike price must be positive, got ${params.strike}`);
    }
    if (params.timeToExpiry < 0) {
      throw new Error(`Time to expiry must be non-negative, got ${params.timeToExpiry}`);
    }
    if (params.volatility < 0) {
      throw new Error(`Volatility must be non-negative, got ${params.volatility}`);
    }
    if (params.dividendYield !== undefined && params.dividendYield < 0) {
      throw new Error(`Dividend yield must be non-negative, got ${params.dividendYield}`);
    }
  }
  ...
  private numericalGreeks(params: OptionParams): Greeks {
    ...
    // Gamma ‚Ä?central second difference
    const gamma = (priceUp - 2 * basePrice + priceDown) / (bumpS * bumpS);

    // Theta ‚Ä?forward difference (time decreases ‚Ü?price generally decreases)
  ...
  // -----------------------------------------------------------------------
  // Implied volatility ‚Ä?bisection fallback
  // -----------------------------------------------------------------------
  ...
  private impliedVolatilityBisection(
    ...
    for (let iter = 0; iter < 200; iter++) {
      ...
      if (Math.abs(diff) < IV_TOLERANCE) {
        log.debug(
          `[OptionsPricingEngine] IV (bisection) converged in ${iter + 1} iterations ‚Ü?œÉ=${mid.toFixed(6)}`,
        );
        return mid;
      }
    ...
    log.warn(
      `[OptionsPricingEngine] IV bisection did not fully converge ‚Ü?œÉ‚â?{finalMid.toFixed(6)}`,
    );
    return finalMid;
  }
}
...
// ‚îÄ‚îÄ Standalone exports for main.ts import ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ‚îÄ
