...
export class PositionMonitor extends EventEmitter {
  ...
  private validatePosition(position: TrackedPosition): void {
    if (!position.code || position.code.trim().length === 0) {
      throw new Error('[PositionMonitor] Position code is required');
    }

    if (position.side !== 'long' && position.side !== 'short') {
      throw new Error(`[PositionMonitor] Invalid side: ${position.side}`);
    }

    if (position.qty <= 0 || !Number.isFinite(position.qty)) {
      throw new Error(`[PositionMonitor] Invalid qty: ${position.qty}`);
    }

    if (position.entryPrice <= 0 || !Number.isFinite(position.entryPrice)) {
      throw new Error(`[PositionMonitor] Invalid entryPrice: ${position.entryPrice}`);
    }

    if (position.entryTime <= 0) {
      throw new Error(`[PositionMonitor] Invalid entryTime: ${position.entryTime}`);
    }

    if (position.stopLoss !== undefined && position.stopLoss <= 0) {
      throw new Error(`[PositionMonitor] Invalid stopLoss: ${position.stopLoss}`);
    }

    if (position.takeProfit !== undefined && position.takeProfit <= 0) {
      throw new Error(`[PositionMonitor] Invalid takeProfit: ${position.takeProfit}`);
    }
    ...
    if (
      ...
      throw new Error(
        `[PositionMonitor] Invalid trailingStopPct: ${position.trailingStopPct} (must be 0-100)`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `[PositionMonitor] Invalid timeExitMinutes: ${position.timeExitMinutes}`,
      );
    }
