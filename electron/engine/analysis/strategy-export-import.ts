...
export class StrategyExportImport extends EventEmitter {
  ...
  registerStrategy(config: StrategyConfig): void {
    if (!config.id || !config.name || !config.engine) {
      throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, 'Strategy must have id, name, and engine');
    }
  ...
  private deserializeJson(data: string): StrategyConfig[] {
    ...
    throw new EngineError(ErrorCode.STRATEGY_CREATE_FAILED, 'Invalid JSON format: expected {strategies: [...]} or [...] or single strategy object');
  }
