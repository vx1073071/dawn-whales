...
export class UnifiedFactorCalculator {
  ...
  // === R278 Calculator 60→80% enhancements ===
  calcBatch(inp: FactorCalcInput, factorIds?: string[]): Map<string, FactorCalcResult> {const ids=factorIds||this.configs.filter(c=>c.enabled).map(c=>c.id);const res=new Map();for(const id of ids){const r=this.calcFactor(id,inp);if(r)res.set(id,r)}return res}
  calcDelta(inp: FactorCalcInput, prevInp: FactorCalcInput): {added:FactorCalcResult[];removed:FactorCalcResult[];changed:FactorCalcResult[]} {const cur=this.calcBatch(inp);const prev=this.calcBatch(prevInp);const a:FactorCalcResult[]=[];const rm:FactorCalcResult[]=[];const ch:FactorCalcResult[]=[];Array.from(cur.entries()).forEach(([id,v])=>{if(!prev.has(id))a.push(v);else if(Math.abs(v.normalized-prev.get(id)!.normalized)>0.001)ch.push(v)});Array.from(prev.keys()).forEach((id)=>{if(!cur.has(id))rm.push(prev.get(id)!)});return{added:a,removed:rm,changed:ch}}
  attribution(inp: FactorCalcInput): Array<{factorId:string;factorName:string;category:string;contribution:number;rawValue:number}> {const results=this.calcAll(inp);const total=results.reduce((s,r)=>s+Math.abs(r.normalized-0.5),0);return results.map(r=>({factorId:r.factorId,factorName:r.factorName,category:r.category,contribution:total>0?Math.abs(r.normalized-0.5)/total:0,rawValue:r.value})).sort((a,b)=>b.contribution-a.contribution)}
  consistencyCheck(factorId: string): {rolling24m:{mean:number;sd:number};count:number;stale:boolean} {const h=this.calcHist.get(factorId);if(!h||h.length<5)return{rolling24m:{mean:0,sd:0},count:h?.length||0,stale:true};const recent=h.slice(-24);const m=recent.reduce((a,b)=>a+b,0)/recent.length;const sd=Math.sqrt(recent.reduce((s,v)=>s+(v-m)**2,0)/recent.length);return{rolling24m:{mean:m,sd},count:h.length,stale:h.length>10&&sd<1e-6}}
  correlationMatrix(): {ids:string[];matrix:number[][]}|null {const ids=this.configs.filter(c=>c.enabled&&this.calcHist.has(c.id)).map(c=>c.id);if(ids.length<2)return null;const n=ids.length;const m=Math.min(...ids.map(id=>this.calcHist.get(id)!.length));if(m<5)return null;const mat:number[][]=Array.from({length:n},()=>Array(n).fill(0));for(let i=0;i<n;i++){const hi=this.calcHist.get(ids[i])!.slice(-m);for(let j=0;j<n;j++){if(i===j){mat[i][j]=1;continue}const hj=this.calcHist.get(ids[j])!.slice(-m);let sxy=0,sx=0,sy=0,sx2=0,sy2=0;for(let k=0;k<m;k++){sx+=hi[k];sy+=hj[k];sx2+=hi[k]**2;sy2+=hj[k]**2;sxy+=hi[k]*hj[k]}const den=Math.sqrt((m*sx2-sx*sx)*(m*sy2-sy*sy));mat[i][j]=den>0?(m*sxy-sx*sy)/den:0}}return{ids,matrix:mat}}
  factorStats(): Array<{id:string;name:string;mean:number;sd:number;sharpe:number;count:number}> {return this.configs.filter(c=>c.enabled).map(c=>{const h=this.calcHist.get(c.id);if(!h||h.length<5)return{id:c.id,name:c.name,mean:0,sd:0,sharpe:0,count:h?.length||0};const recent=h.slice(-60);const m=recent.reduce((a,b)=>a+b,0)/recent.length;const sd=Math.sqrt(recent.reduce((s,v)=>s+(v-m)**2,0)/recent.length);return{id:c.id,name:c.name,mean:m,sd,sharpe:sd>0?m/sd:0,count:h.length}}).sort((a,b)=>b.sharpe-a.sharpe)}
