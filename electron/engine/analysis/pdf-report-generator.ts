...
<div class="footer">
  QUANT MOO Report Generator &mdash; Confidential
</div>
</body>
</html>`;
}
...
export function buildEmailMessage(
  ...
  <p style="color:#999;font-size:11px;">Generated by QUANT MOO Report Generator</p>
</div>`,
    attachments: [
      {
...
export function createSmtpTransporter(config: EmailConfig): {
  ...
  return {
    async sendMail(message: EmailMessage): Promise<{ success: boolean; messageId: string }> {
      ...
      const messageId = `msg_${Date.now()}_${Math.random().toString(36).slice(2, 8)}@quant-moo`;
