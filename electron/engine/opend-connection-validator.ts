...
export class OpenDConnectionValidator {
   ...
   */
  async testGetGlobalState(): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
   ...
   */
  async testGetSnapshot(symbol: string = 'HK.00700'): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      const { ipcRenderer } = require('electron');
