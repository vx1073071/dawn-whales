...
export class DataWarehouse {
  ...
  insert(table: string, rows: any[]): void {
    ...
    if (!tbl) {
      throw new Error(`[DataWarehouse] Table "${table}" does not exist`);
    }
  ...
  upsert(table: string, rows: any[], keyColumn: string): void {
    ...
    if (!colDef) {
      throw new Error(`[DataWarehouse] Key column "${keyColumn}" not found in table "${table}"`);
    }
  ...
  query<T = any>(options: QueryOptions): QueryResult<T> {
  ...
  setPartitionConfig(config: PartitionConfig): void {
    ...
    if (!tbl) {
      throw new Error(`[DataWarehouse] Table "${config.table}" does not exist`);
    }
    ...
    if (!colDef || colDef.type !== 'datetime') {
      throw new Error(
        `[DataWarehouse] Partition time column "${config.timeColumn}" must be of type 'datetime'`
      );
    }
  ...
  importTable(table: string, data: string, format: 'json' | 'csv'): number {
    ...
    if (format === 'json') {
      ...
      if (!Array.isArray(rows)) {
        throw new Error('[DataWarehouse] JSON import expects an array of rows');
      }
    } else {
      // CSV import
      const lines = data.split(/\r?\n/).filter((l) => l.trim().length > 0);
      if (lines.length < 2) {
        throw new Error('[DataWarehouse] CSV import requires a header row and at least one data row');
      }
  ...
  addIndex(table: string, column: string): void {
    ...
    const colDef = tbl.columns.find((c) => c.name === column);
    if (!colDef) {
      throw new Error(`[DataWarehouse] Column "${column}" not found in table "${table}"`);
    }
  ...
  private coerceValue(val: any, colDef?: ColumnDef): any {
