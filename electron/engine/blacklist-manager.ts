...
export class BlacklistManager {
  ...
  add(userId: string, reason: string, addedBy: string, expiresInDays?: number, notes?: string): BlacklistEntry {
    if (this.whitelist.has(userId)) throw new Error(`User ${userId} is whitelisted, cannot blacklist`);
  ...
  remove(userId: string, removedBy: string): BlacklistEntry {
    ...
    if (!entry) throw new Error(`User ${userId} is not blacklisted`);
    if (entry.status !== 'active') throw new Error(`Blacklist entry for ${userId} is ${entry.status}`);
