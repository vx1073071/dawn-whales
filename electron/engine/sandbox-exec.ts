...
function validateInputs(args: string[]): void {
  ...
  if (joined.length > MAX_INPUT_LENGTH) {
    throw new Error('Sandbox: Input exceeds max length');
  }
  
  for (const arg of args) {
    if (Buffer.byteLength(arg, 'utf8') > 4096) {
      throw new Error('Sandbox: Argument exceeds limit');
    }
  }
  
  for (const pattern of FORBIDDEN_PATTERNS) {
    if (pattern.test(joined)) {
      throw new Error(`Sandbox: Forbidden pattern detected: ${pattern}`);
    }
  }
}
...
function checkAllowlist(command: string, skipAllowlist: boolean): void {
  ...
  throw new Error(`Sandbox: Command "${command}" is not in the allowlist`);
}
