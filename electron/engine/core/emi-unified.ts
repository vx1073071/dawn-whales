  ...
  try {
    ...
    if (fundResult.success) {
      ...
      result.fundHoldings = {
        ...
        totalShares: items.reduce((s: number, i: unknown) => s + (i.shares || 0), 0),
        increaseCount: items.filter((i: unknown) => (i.sharesChange || 0) > 0).length,
        decreaseCount: items.filter((i: unknown) => (i.sharesChange || 0) < 0).length,
  ...
  try {
    ...
    if (newsResult.success) {
      result.news = {
        ...
        topHeadlines: newsResult.articles.slice(0, 5).map((a: unknown) => a.title),
      };
    }
  } catch (err: unknown) {
...
export async function getMarketOverview(): Promise<MarketOverview> {
  ...
  try {
    ...
    if (heatmap.success) {
      result.topSectors = heatmap.sectors.slice(0, 5).map((s: unknown) => ({
        name: s.name,
  ...
  try {
    ...
    if (inflowResult.success) {
      result.capitalFlow.topInflowSectors = inflowResult.items.slice(0, 5).map((s: unknown) => s.name);
    }
    if (outflowResult.success) {
      result.capitalFlow.topOutflowSectors = outflowResult.items.slice(0, 5).map((s: unknown) => s.name);
    }
  } catch (err: unknown) {
...
export async function getDailyReport(): Promise<DailyReport> {
  ...
  try {
    ...
    logger.error('[backend:emi-unified]', e); }
