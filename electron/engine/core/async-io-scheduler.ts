...
export class AsyncIOScheduler {
  ...
  submit(taskInput: Omit<IOTask, 'id' | 'createdAt'>): string {
    if (this.disposed) {
      throw new EngineError(ErrorCode.MONITORING_ERROR, 'AsyncIOScheduler is disposed');
    }
    if (this.queue.size >= this.config.maxQueueSize) {
      ...
      throw new EngineError(ErrorCode.MONITORING_ERROR, `Scheduler queue full (${this.queue.size}/${this.config.maxQueueSize}). Backpressure applied.`,);
    }
