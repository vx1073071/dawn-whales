...
export class BayesianOptimizer {
  ...
  suggest(): Record<string, number> {
    if (this.parameters.length === 0) {
      throw new Error('[BayesianOptimizer] No parameters configured. Call optimize() first or set parameters via observe() context.');
    }
  ...
  private validateParameters(params: Parameter[]): void {
    if (params.length === 0) {
      throw new Error('[BayesianOptimizer] At least one parameter is required');
    }
    for (const p of params) {
      if (p.type === 'categorical') {
        if (!p.values || p.values.length === 0) {
          throw new Error(`[BayesianOptimizer] Categorical parameter "${p.name}" must have values`);
        }
      } else {
        if (p.min >= p.max) {
          throw new Error(`[BayesianOptimizer] Parameter "${p.name}": min must be < max (got min=${p.min}, max=${p.max})`);
        }
      }
    }
  }
  ...
  private paramsToVec(params: Record<string, number>): number[] {
    return this.parameters.map((p) => {
      ...
      if (val === undefined) {
        throw new Error(`[BayesianOptimizer] Missing parameter "${p.name}"`);
      }
      return val;
    });
  }
  ...
  suggestBatch(batchSize: number): Record<string, number>[] {
    if (this.parameters.length === 0) {
      throw new Error('[BayesianOptimizer] No parameters configured');
    }
  ...
  optimizeMultiObjective(
    ...
    if (objectives.length !== weights.length) {
      throw new Error('[BayesianOptimizer] objectives and weights must have same length');
    }
