...
export class StrategyMarketplaceEngine {
  ...
  createListing(params: {
    ...
    if (params.price < 1 || params.price > 1000) throw new Error('Price must be 1-1000 USDT');
    if (params.name.length < 3) throw new Error('Name too short (min 3 characters)');
    if (params.description.length < 10) throw new Error('Description too short (min 10 characters)');
  ...
  updateListing(listingId: string, updates: { name?: string; description?: string; price?: number }): StrategyListing {
    const listing = this.listings.get(listingId);
    if (!listing) throw new Error('Listing not found');
    if (updates.price !== undefined && (updates.price < 1 || updates.price > 1000)) throw new Error('Price must be 1-1000 USDT');
    if (updates.name !== undefined && updates.name.length < 3) throw new Error('Name too short');
    if (updates.description !== undefined && updates.description.length < 10) throw new Error('Description too short');
