...
export class BillingWalletServer {
  ...
  createAccount(userId: string, email: string, tier: PricingTier = 'basic'): WalletAccount {
    if (this.accounts.has(userId)) throw new Error('Account already exists');
  ...
  freezeBalance(userId: string, amount: number, reason: string): Transaction {
    const account = this.accounts.get(userId);
    if (!account) throw new Error('Account not found');
    if (account.balance - account.frozenBalance < amount) throw new Error('Insufficient available balance');
  ...
  unfreezeBalance(userId: string, amount: number, reason: string): Transaction {
    const account = this.accounts.get(userId);
    if (!account) throw new Error('Account not found');
    if (account.frozenBalance < amount) throw new Error('Insufficient frozen balance');
  ...
  requestWithdrawal(userId: string, amount: number, toAddress: string): WithdrawalRequest {
    const account = this.accounts.get(userId);
    if (!account) throw new Error('Account not found');
    if (amount < this.config.minWithdrawalAmount) throw new Error(`Min withdrawal: ${this.config.minWithdrawalAmount} USDT`);
    if (account.balance - account.frozenBalance < amount) throw new Error('Insufficient balance');
  ...
  approveWithdrawal(requestId: string, reviewer: string): WithdrawalRequest {
    ...
    if (req.status !== 'pending') throw new Error(`Cannot approve ${req.status} withdrawal`);
  ...
  rejectWithdrawal(requestId: string, reviewer: string, reason: string): WithdrawalRequest {
    const req = this.withdrawals.get(requestId);
    if (!req) throw new Error('Withdrawal not found');
    if (req.status !== 'pending') throw new Error(`Cannot reject ${req.status} withdrawal`);
