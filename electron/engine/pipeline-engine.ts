...
export interface PipelineStep {
  ...
  fn: (input: any[], context: PipelineContext) => Promise<any[]>;
}
...
export class PipelineEngine {
  ...
  addStep(pipelineId: string, step: PipelineStep): void {
    ...
    if (def.steps.some((s) => s.id === step.id)) {
      throw new Error(`Step "${step.id}" already exists in pipeline "${pipelineId}"`);
    }
...
export const StepTemplates = {
  ...
  map(
    id: string,
    name: string,
    mapper: (row: any, index: number) => any,
    config: Record<string, any> = {},
  ): PipelineStep {
    return {
      id,
  ...
  dedupe(
    ...
    return {
      ...
      fn: async (input: any[]) => {
        const getKey = keyFn ?? ((row: any) => JSON.stringify(row));
  ...
  validate(
    id: string,
    name: string,
    validator: (row: any) => { valid: boolean; reason?: string },
    ...
    return {
      ...
      fn: async (input: any[], context: PipelineContext) => {
        const results: any[] = [];
        for (const row of input) {
  ...
  enrich(
    id: string,
    name: string,
    enricher: (row: any, context: PipelineContext) => Promise<Record<string, any>>,
    config: Record<string, any> = {},
  ): PipelineStep {
    return {
      id,
  ...
  load(
    id: string,
    name: string,
    loader: (data: any[], context: PipelineContext) => Promise<void>,
    config: Record<string, any> = {},
  ): PipelineStep {
    return {
      id,
  ...
  tap(
    id: string,
    name: string,
    callback: (data: any[], context: PipelineContext) => void,
    config: Record<string, any> = {},
  ): PipelineStep {
    return {
      id,
